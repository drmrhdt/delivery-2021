      ...
      <div class="modal-card" style="width: auto">
        <header class="modal-card-head">
          <p class="modal-card-title mr-5">
            Подтвердите передачу документов отправителем курьеру
          </p>
          <button type="button" class="delete" @click="$emit('close')" />
        </header>
        <section class="modal-card-body">
          <div class="flexxx">
            <canvas style="width: 100%" id="canvas1"></canvas>
            <canvas style="width: 100%" id="canvas2"></canvas>
          </div>
          <div class="flexxx">
            <p class="subtitle is-5">Подпись отправителя</p>
  ...
  methods: {
    ...
    confirm() {
      if (!signaturePad.isEmpty() && !signaturePad2.isEmpty()) {
        ...
        console.log(`Подпись отправителя:`, code1);
